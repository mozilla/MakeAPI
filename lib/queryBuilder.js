    ...
    build: function( query, callback ) {
      ...
      Object.keys( query ).forEach(function( key ){
        ...
        if ( generators[ key ] ) {
          if ( NOT_REGEX.test( value ) ) {
            searchQuery.query.filtered.filter.and.push( generators[ key ]( value.substring( 3 ), true ) );
          } else {
            searchQuery.query.filtered.filter.and.push( generators[ key ]( value ) );
          }
        }
      });
      ...
      if ( user ) {
        ...
        loginApi.getUser( user, function( err, userData ) {
          ...
          searchQuery.query.filtered.filter.and.push( filter );
