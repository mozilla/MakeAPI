...
module.exports = function( loginApi ) {
  ...
  var generators = {
    ...
    var generateRange = function ( field , gte, lt){
        var filter = {};
            filter[""+field] = { "gte": gte ,
                                  "lt": lt };
          return {"range":filter};
     };
    var generateBool = function( Must, Should, Must_Not ) {
        return { bool : { must : Must, should : Should, must_not : Must_Not } };
     };   
    var generateMissing = function( Field, isNull, isExist ) {
      ...
      var baseQuery = {
         ...
         if ( !! query.metric ) {
            if ( query.metric == 'all' ){
              ...
              baseQuery.query.filtered.filter = generateBool( generateRange( "createdAt", today(), nextDay(1) ), 
                                                              generateMissing( "remixedFrom", true, true) ,{}); 
            
            } else if ( query.metric == 'week' ){
              baseQuery.query.filtered.filter = generateBool( generateRange("createdAt", prevDay(6), nextDay(1) ),
                                                              generateMissing("remixedFrom",true,true), {} );
              ...
              baseQuery.query.filtered.filter = generateBool( generateRange( "createdAt", today(), nextDay(1) ), 
                                                              {}, generateMissing( "remixedFrom", false, true ) ); 
