  ...
  return {
    build: function( query, callback ) {
      ...
      var baseQuery = {
         ...
         if ( !!query.metric ) {
            if ( query.metric == 'all' ) {
              baseQuery.query.filtered.filter = generateMissing( "remixedFrom", true, true ) ; 
            } else if ( query.metric == 'day' ) {
              baseQuery.query.filtered.filter = generateBool( generateRange( "createdAt", today(), nextDay(1) ), generateMissing( "remixedFrom", true, true) ,{}); 
            } else if ( query.metric == 'week' ) {
              baseQuery.query.filtered.filter = generateBool( generateRange("createdAt", prevDay(6), nextDay(1) ), generateMissing("remixedFrom",true,true), {} );
            } else if ( query.metric=='remixAll' ) {
              baseQuery.query.filtered.filter = generateBool( {}, {}, generateMissing("remixedFrom",false,true) ); 
            } else if ( query.metric == 'remixDay' ) {
              baseQuery.query.filtered.filter = generateBool( generateRange( "createdAt", today(), nextDay(1) ), {}, generateMissing( "remixedFrom", false, true ) ); 
            } else if ( query.metric == 'remixWeek' ) {
              baseQuery.query.filtered.filter = generateBool( generateRange( "createdAt", prevDay(6), nextDay(1) ), {}, generateMissing( "remixedFrom", false, true ) ); 
            }
         }
    ...
    // If the request contains any of the filter generating keys, or defines a user search, use the advancedQuery object
      ...
      if ( user ) {
        ...
        loginApi.getUserByUsername( user, function( err, userData ) {
          if ( err ) {
            callback({
              error: err,
              code: 500
