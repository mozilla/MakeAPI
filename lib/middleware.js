  ...
  return {
    validateAppTags: function( req, res, next ) {
      ...
      if ( makeTags && makeTags.length ) {
        validTags = tags.validateTags( makeTags, req.user, req.credentials.admin );
      }

      // keep app tags that aren't in ValidTags
      if ( req.make ) {
        validTags = validTags.concat( req.make.appTags.filter(function( tag, pos, arr ) {
    ...
    checkOwnerApp: function( modelName ) {
      if ( [ "make","list" ].indexOf( modelName ) === -1 ) {
        throw new Error( "checkOwnerApp middleware can only be configured with 'make' or 'list'. You passed in: '" + modelName + "'" );
      }
    ...
    getList: function( req, res, next ) {
      ...
      List.findById( req.params.id, function( err, list ) {
        if ( err ) {
          if ( err.name === "CastError" ) {
            return hawkModule.respond( 400, res, req.credentials, req.artifacts, { status: "failure", reason: "The supplied value does not look like a List ID." }, "application/json" );
          } else {
            return hawkModule.respond( 500, res, req.credentials, req.artifacts, { status: "failure", reason: err.toString() }, "application/json" );
          }
        }
        if ( !list ) {
          return hawkModule.respond( 400, res, req.credentials, req.artifacts, { status: "failure", reason: "List Does Not Exist" }, "application/json" );
        }
        req.list = list;
        next();
      });
