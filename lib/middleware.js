...
module.exports = function( makeModel, loginApi, env ) {
  ...
  return {
    ...
    getMake: function( req, res, next ) {
      if ( !req.params.id ) {
        return res.json( 400, { error: "ID missing"} );
      }
      Make.findById( req.params.id ).where( "deletedAt", null ).exec(function( err, make ) {
        if ( err ) {
          if ( err.name === "CastError" ) {
            return res.json( 400, { error: "The supplied value does not look like a Make ID." } );
          } else {
            return res.json( 500, { error: err.toString() } );
          }
        }
        if ( !make ) {
          return res.json( 400, { error: "Make Does Not Exist" } );
        }
        req.make = make;
        next();
      });
