...
module.exports = function( generators ) {
  ...
  //DATE FUNCTIONS TO GET DATES FOR METRICS
  var newDateZero = function () {
         var dtx = new Date();
         dtx.setHours(0);
         dtx.setMinutes(0);
         dtx.setSeconds(0);
     ...
     var prevDay = function ( n ) {
       ...
       today = function () {
         return newDateZero().getTime();
       },
       generateRange = function ( field, gte, lt ) {
         var filter = {};
         filter["" + field] = {
           "gte": gte,
           "lt": lt
       ...
       generateBool = function ( Must, Should, Must_Not ) {
         return {
           bool: {
             must: Must,
             should: Should,
       ...
       generateMissing = function ( Field, isNull, isExist ) {
         return {
           "missing": {
             field: Field,
             existence: isExist,
  ...
  return function( query, callback ) {
    ...
    if ( queryKeys.some( hasValidField ) || user ) {
		// Added metrics feature for webmaker.org
        searchQuery = generators.queries.advancedQuery();
		if( !! query.metric ) {
            if ( query.metric == 'day' ) {
                  searchQuery.query.filtered.filter = generateBool( generateRange( "createdAt", today(), nextDay(1) ), generateMissing( "remixedFrom", true, true) ,{}); 
                } else if ( query.metric == 'week' ) {
                  searchQuery.query.filtered.filter = generateBool( generateRange("createdAt", prevDay(6), nextDay(1) ), generateMissing("remixedFrom",true,true), {} );
                } else if ( query.metric=='remixAll' ) {
                  searchQuery.query.filtered.filter = generateBool( {}, {}, generateMissing("remixedFrom",false,true) ); 
                } else if ( query.metric == 'remixDay' ) {
                  searchQuery.query.filtered.filter = generateBool( generateRange( "createdAt", today(), nextDay(1) ), {}, generateMissing( "remixedFrom", false, true ) ); 
                } else if ( query.metric == 'remixWeek' ) {
                  searchQuery.query.filtered.filter = generateBool( generateRange( "createdAt", prevDay(6), nextDay(1) ), {}, generateMissing( "remixedFrom", false, true ) ); 
                } else if (query.metric == 'all'){
                  searchQuery.query.filtered.filter = generateMissing( "remixedFrom", true, true ) ; 
                }
